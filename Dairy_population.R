...
##################################################
iCalf = 200 ###init # of calf
iHeifer_first_lact = 30  # init # of first lactation cows
iHeifer_second_lact = 21 # init # of second lactation cows
iHeifer_third_lact =15 # init # of third lactation cows
iHeifer_first_dry = 20 # init # of cows dry after first lactation 
iHeifer_second_dry = 15 # init # of cows dry after sec lactation
iHeifer_third_dry =10 # init # of number of third lactation dry
iLact = 45 # init # of number of fourth lactation or greater, cows
iDry =30 # init # of cow number fourth lactation or greater, dry cows
iyearling=38 # init # of yearling heifers
ibredHeifer=120 # init # of bred heifer
iBLH=72 # init # of bred lactating heifers
iBredSecLC=54 # init # of second lactation cows
iBredThirdLC=38 # init # of third lactation bred 3rd lactation cows
iBredLC=115 # init # of bred lactation cows
###################################################
...
###################################################################
Animal_df= data.frame(matrix(ncol=53, nrow=nrow(Date)))
...
Animal_df$total_number[1]=iCalf+iyearling+ibredHeifer+iHeifer_first_lact+iBLH+
  iHeifer_first_dry+iHeifer_second_lact+iBredSecLC+iHeifer_second_dry+iHeifer_third_lact+
  iBredThirdLC+iHeifer_third_dry+iLact+iBredLC+iDry
Animal_df$calves_born[1]=0 #assume that calves born in day 1  is 0
...
target_population=800 ##capacity of the farm to keep 
...
for (i in (1:(nrow(Date)))){
  ...
  if (Animal_df$total_number[i]>target_population){
    Animal_df$excessive_cull[i]=(Animal_df$total_number[i]-target_population)/5 # the excessive will be devided between yearling, lactating heifers,
    #2nd lactation cows, 3rd lac cows,lactating cows
    Animal_df$total_number[i]=target_population
    
  }else{
    Animal_df$excessive_cull[i]=0
  }
  ...
  ###################### lactating cows, bred lactating cows, dry cows
  ...
  if (i+1 <= nrow(Date)){
    Animal_df$LC_to_BredLC[i+1]=(Animal_df$LC[i]/85)##Lactating cows to bred lactating cows
    
    Animal_df$BredLC_to_DC[i+1]=Animal_df$BredLC[i]/220
    Animal_df$DC_to_LC[i+1]=Animal_df$DC[i]/60 ## dry cow to lactating cows
    
    Animal_df$calves_born[i+1]=(Animal_df$DB1HC[i]+Animal_df$DBHC[i]+Animal_df$DB2LCC[i]+Animal_df$DB3LCC[i]+Animal_df$BCC[i])*PropF*PropKeep#CALVES BORN
    if (Animal_df$calves_born[i+1]<0){
      Animal_df$calves_born[i+1]=0
    }
    Animal_df$calves[i+1]=Animal_df$calves[i]+Animal_df$calves_born[i+1]-Animal_df$calves_death[i]-Animal_df$calves_to_yearling[i]##calf pool
    if (Animal_df$calves[i+1]<0){
      Animal_df$calves[i+1]=0
    }
    Animal_df$yearling[i+1]=Animal_df$yearling[i]+Animal_df$calves_to_yearling[i]-Animal_df$yearling_death[i]-Animal_df$yearling_to_bredHeifers[i]-Animal_df$excessive_cull[i]-Animal_df$yearling_cull[i]#yearling pool
    ...
    Animal_df$LactatingHeifers[i+1]=Animal_df$LactatingHeifers[i]+Animal_df$bredHeifer_to_LactHeif[i]-Animal_df$LH_to_BLH[i]-Animal_df$lactatingHeifers_death[i]-Animal_df$excessive_cull[i]-Animal_df$LactHeif_cull[i] ##lactating heifer pool
    ...
    Animal_df$SecondLC[i+1]=Animal_df$SecondLC[i]+Animal_df$DBH_to_SecondLC[i]-Animal_df$SecondLC_death[i]-Animal_df$SecondLC_to_BredSecLC[i]-Animal_df$excessive_cull[i]-Animal_df$SecondLC_cull[i]  ##2nd lactation cows
    ...
    Animal_df$ThirdLC[i+1]=Animal_df$ThirdLC[i]+Animal_df$DSecondLC_to_ThirdLC[i]-Animal_df$ThirdLC_death[i]-Animal_df$ThirdLC_to_BredThirdLC[i]-Animal_df$excessive_cull[i]-Animal_df$ThirdLC_cull[i]#3rd lactation cows
    ...
    Animal_df$LC[i+1]=Animal_df$LC[i]+Animal_df$DThirdLC_to_LC[i]-Animal_df$LC_to_BredLC[i]-Animal_df$LC_death[i]+Animal_df$DC_to_LC[i]-Animal_df$excessive_cull[i]-Animal_df$LC_cull[i] ##lactating cows
